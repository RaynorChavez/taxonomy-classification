...
def predict_one_image(image_file, model, processor, class_names, k):
    ...
    eval_sentences = ["Photograph of a {:s}".format(ct) for ct in class_names]
...
fres = open(os.path.join(
    "data", "results", get_model_basename(args.model_dir) + "new_test_preds.tsv"), "w")
num_predicted = 0
...
fres = open(os.path.join("data", "results", model_basename + "new_test_preds.tsv"), "r")
for line in fres:
...
fscores = open(os.path.join("data", "results", model_basename + "new_test_score.tsv"), "w")
fscores.write("{:s}\t{:s}\n".format(
    model_basename, 
